...
OUT = './output/r2s_2026_PT-55'

res = 100
orifice_radii = np.linspace(0.0002, 0.002, res)
nozzle_radii = np.linspace(.002, .04, res)

thrust = np.zeros(res*res)
isp = np.zeros(res*res)
impulse = np.zeros(res*res)
ox_usage = np.zeros(res*res)
burn_time = np.zeros(res*res)
...
for orifice_r in orifice_radii:
    for nozzle_r in nozzle_radii:
        ...
        engine.ox_tank_volume = 25/1000
        engine.ox_initial_temp_C = 20
        engine.ox_orifice_number = 5
    ...
    burn_time_reshape = burn_time.reshape(res, res)
...
plt.subplot(3, 2, 5)
add_colorbar(burn_time, 'summer', 'Burn time (s)')
plt.contourf(orifice_radii*1000, nozzle_radii*1000, burn_time_reshape, 100, cmap='summer')
plt.xlabel('Ox Orifice radius (mm)')
plt.ylabel('Nozzle Throat radius (mm)')

os.makedirs(OUT, exist_ok=True)
plt.savefig(f'{OUT}/explore_engine.png', bbox_inches='tight')
